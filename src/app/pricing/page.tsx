...
const INDIVIDUAL_PLANS = [
  ...
  },
  {
    ...
    price: "₹2,499",
    period: "/month",
    description: "Access 7,000+ courses. Earn unlimited certificates in the short term.",
    highlight: true,
    badge: "Most Popular",
...
function PlanCard({ plan, i }: { plan: (typeof INDIVIDUAL_PLANS)[0]; i: number }) {
  ...
  return (
    <motion.div
      ...
      className={`relative rounded-2xl flex flex-col overflow-hidden transition-all duration-300 ${
        hasHighlight ? "ring-2 ring-violet-500/60 shadow-2xl shadow-violet-500/20" : "border border-white/[0.08]"
      }`}
      style={{
          ...
          {plan.features.map((f) => (
            <div key={f} className="flex items-center gap-3">
              <div className={`w-4 h-4 rounded-full flex-shrink-0 flex items-center justify-center ${
                ...
                <Check size={10} className={hasHighlight ? "text-violet-400" : "text-gray-500"} strokeWidth={3} />
              </div>
...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div className="border-b border-white/[0.05] last:border-0">
      ...
      >
        <span className="text-base font-bold text-white group-hover:text-violet-400 transition-colors pr-4">{q}</span>
        <ChevronDown
          size={18}
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <p className="text-gray-400 text-sm pb-6 leading-relaxed max-w-3xl">{a}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 pt-28 pb-32">
        ...
        <div className="text-center mb-14">
          ...
          <motion.h1
            ...
            className="text-5xl md:text-6xl font-black mb-5 leading-[1.1] tracking-tight"
          >
            Flexible Plans for <br />
            <span className="bg-gradient-to-r from-violet-400 to-purple-500 bg-clip-text text-transparent">Every Career Level</span>
          </motion.h1>
          ...
          >
            All plans include a 7-day free trial. No hidden fees, cancel any time.
          </motion.p>
        </div>
        ...
        <div className="mb-24">
          <div className="text-center mb-12">
            <p className="text-xs font-black tracking-[0.2em] uppercase text-gray-500 mb-3">Compare Plans</p>
            <h2 className="text-3xl font-black text-white">What's included in each plan</h2>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  <th className="py-5 px-6 text-left text-xs font-black text-gray-500 uppercase tracking-widest">Feature</th>
                  <th className="py-5 px-4 text-center text-xs font-black text-gray-400 uppercase tracking-widest">Single</th>
                  <th className="py-5 px-4 text-center text-xs font-black text-violet-400 uppercase tracking-widest">Plus</th>
                  <th className="py-5 px-4 text-center text-xs font-black text-blue-400 uppercase tracking-widest">Enterprise</th>
                </tr>
              </thead>
                  ...
                  <tr key={row.feature} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    {[row.free, row.plus, row.ent].map((val, idx) => (
                      <td key={idx} className="py-4 px-4 text-center">
                        {val ? (
                          <Check size={16} className={idx === 1 ? "text-violet-400 mx-auto" : idx === 2 ? "text-blue-400 mx-auto" : "text-gray-500 mx-auto"} />
                        ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-16 mb-24">
          <div>
            ...
            <h2 className="text-3xl font-black text-white mb-4 leading-tight">Frequently<br />Asked Questions</h2>
            <p className="text-gray-500 text-sm mb-6 leading-relaxed">
              Can't find what you're looking for? Contact our student support team.
            </p>
            <Link href="/support" className="inline-flex items-center gap-2 text-sm font-bold text-violet-400 hover:text-violet-300 group">
        ...
        <div className="relative rounded-3xl overflow-hidden p-12 lg:p-20 text-center border border-white/[0.06] bg-gradient-to-br from-[#0c0c1a] via-[#0a0a14] to-[#060610]">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-4xl md:text-5xl font-black mb-5">Invest in Your Future Today</h2>
            <p className="text-gray-400 text-base max-w-xl mx-auto mb-10 leading-relaxed">
              Join thousands of professionals accelerating their careers with EduNova's specialized learning tracks.
            </p>
