...
function planToEnumAndMonths(plan, isFree) {
    ...
    if (plan === "plus" || plan === "teams" || plan === "teams-pro")
        return { planEnum: PlanDuration.ONE_MONTH, months: 1 };
    if (plan === "3month")
        return { planEnum: PlanDuration.THREE_MONTH, months: 3 };
    if (plan === "6month" || plan === "annual")
        return { planEnum: PlanDuration.SIX_MONTH, months: (plan === "annual" ? 12 : 6) };
    return { planEnum: PlanDuration.ONE_MONTH, months: 1 };
}
function getAmountForPlan(plan, isFree, prices) {
    ...
    if (plan === "plus")
        return 22.33; // ₹2,099 / 94
    if (plan === "annual")
        return 212.75; // ₹19,999 / 94
    if (plan === "teams")
        return 21.27; // ₹1,999 / 94
    if (plan === "teams-pro")
        return 15.95; // ₹1,499 / 94
    if (plan === "3month")
