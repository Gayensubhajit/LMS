...
export async function handleRazorpayWebhook(req, res) {
    ...
    const paymentStatus = status === "captured" ? PaymentStatus.SUCCESS : status === "failed" ? PaymentStatus.FAILED : PaymentStatus.CREATED;
...
export async function handleStripeWebhook(req, res) {
    ...
    if (event.type === "payment_intent.succeeded" || event.type === "payment_intent.payment_failed") {
        ...
        if (payment) {
            const nextStatus = event.type === "payment_intent.succeeded" ? PaymentStatus.SUCCESS : PaymentStatus.FAILED;
            await prisma.paymentOrder.update({
                where: { id: payment.id },
                data: {
