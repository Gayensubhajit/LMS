...
const LeaderboardCard = ({ user, index }: Props) => {
  ...
  return (
    <Link href={`/students/${user.id}`} className="block relative group">
      <motion.div
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ delay: index * 0.05 }}
        className={`relative p-4 rounded-2xl flex items-center gap-4 transition-all duration-300 border ${
          user.isCurrent 
            ? "bg-indigo-600/10 border-indigo-500/30 dark:bg-violet-600/10 dark:border-violet-500/30" 
            : "bg-white dark:bg-white/[0.02] border-slate-200 dark:border-white/5 group-hover:border-violet-500/30 shadow-sm group-hover:shadow-md"
        }`}
      >
        {/* Rank Badge */}
        ...
        <div className="w-12 h-12 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 shrink-0 overflow-hidden">
          {user.avatar ? (
            ...
            <div className="w-full h-full flex items-center justify-center font-black text-slate-400 dark:text-gray-600 uppercase">
              {user.name[0]}
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h4 className="font-bold text-slate-900 dark:text-white truncate group-hover:text-indigo-600 dark:group-hover:text-violet-400 transition-colors">
              {user.name}
              {user.isCurrent && (
          ...
          <div className="flex items-center gap-1 mt-1 text-[10px] font-black text-slate-400 dark:text-gray-500 uppercase tracking-widest">
