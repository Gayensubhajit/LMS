...
import { getUserSettings, updateUserSettings, UserSettings } from "@/lib/settings-api";
          ...
          <aside className="w-full lg:w-72 shrink-0">
            <div className="bg-white/5 border border-white/10 rounded-3xl p-3 backdrop-blur-xl sticky top-32">
              <div className="p-4 mb-2">
                 <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Settings Hub</h2>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Icon size={18} className={isActive ? "text-violet-400" : "text-gray-500 group-hover:text-gray-400"} />
                        <span className="text-sm font-bold tracking-tight">{item.label}</span>
                      </div>
                      {isActive && <motion.div layoutId="activeDot" className="w-1.5 h-1.5 rounded-full bg-violet-400 shadow-[0_0_8px_rgba(167,139,250,0.8)]" />}
                    </button>
                  );
                })}
              </nav>
            ...
            <AnimatePresence mode="wait">
              {activeTab === "account" && (
                ...
                >
                  <div className="flex justify-between items-end mb-2">
                    <h1 className="text-3xl md:text-4xl font-black tracking-tighter uppercase italic text-white/90">Account <span className="text-violet-500">Settings</span></h1>
                    {success && (
                      <motion.div 
                  ...
                  <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-8 md:p-12 backdrop-blur-3xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row gap-10 items-start">
                       ...
                       <div className="flex flex-col items-center gap-4">
                          ...
                          <button className="text-[10px] font-black uppercase tracking-widest text-violet-400 hover:text-white transition-colors">Change Photo</button>
                       </div>
                       ...
                       <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                          ...
                          <div className="space-y-2">
                             <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-1">Timezone</label>
                             <div className="relative group/input">
                                ...
                                <Clock size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within/input:text-violet-400 transition-colors" />
                             </div>
                          </div>
                          <div className="space-y-2">
                             <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-1">Preferred Language</label>
                             <div className="relative group/input">
                                <select 
                                ...
                                <Languages size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within/input:text-violet-400 transition-colors" />
                             </div>
                          </div>
                       </div>
                    </div>
                    ...
                    <div className="mt-12 pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
                       <div className="flex items-center gap-4">
                          <div className={`p-2.5 rounded-xl border ${settings?.isNameVerified ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400" : "bg-amber-500/10 border-amber-500/30 text-amber-400"}`}>
                             {settings?.isNameVerified ? <CheckCircle2 size={20} /> : <AlertCircle size={20} />}
                          </div>
                          <div className="space-y-1">
                             <p className="text-sm font-black uppercase tracking-tight">Verified Name for Certificates</p>
                             <p className="text-xs text-gray-500 font-medium">{settings?.isNameVerified ? `Verified as: ${settings.verifiedName}` : "Not yet verified. Certificates will use your account name."}</p>
                          </div>
                       </div>
                       <button 
                  ...
                  <div className="pt-8">
                     <div className="bg-red-500/5 border border-red-500/20 rounded-[2.5rem] p-8 md:p-12 backdrop-blur-sm flex flex-col md:flex-row justify-between items-center gap-8">
                        <div className="space-y-2 text-center md:text-left">
                           <h3 className="text-xl font-black uppercase tracking-tight text-red-100">Deactivate Interstellar Core</h3>
                           <p className="text-sm text-red-500/60 max-w-md font-medium">Permanently delete your EduNova profile and all acquisition logs. This action is irreversible.</p>
                        </div>
              ...
              {activeTab === "security" && (
                <motion.div
                  ...
                  className="space-y-8"
                >
                   <h1 className="text-3xl md:text-4xl font-black tracking-tighter uppercase italic text-white/90">Security <span className="text-violet-500">Vault</span></h1>
                   
                   <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-10 backdrop-blur-xl">
                      <div className="flex items-center justify-between">
                         <div className="space-y-2">
                             <h3 className="text-lg font-black uppercase tracking-tight">Two-Factor Authentication</h3>
                             <p className="text-sm text-gray-500 font-medium max-w-sm">Secure your educational voyages with an extra layer of protection using your mobile device.</p>
                         </div>
                         <button 
                           onClick={() => handleUpdate({ twoFactorEnabled: !settings?.twoFactorEnabled })}
                   ...
                   <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-10 backdrop-blur-xl space-y-8">
                      <h3 className="text-lg font-black uppercase tracking-tight">Connected Nodes</h3>
                      <div className="space-y-4">
                         {["Google", "Apple", "GitHub"].map((node) => (
                           <div key={node} className="flex items-center justify-between py-4 border-b border-white/5 last:border-0">
                              <div className="flex items-center gap-4">
                                 <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center">
                                    <Globe size={18} className="text-gray-400" />
                                 </div>
                                 <span className="font-bold tracking-tight">{node}</span>
                              </div>
                              <button className="text-[10px] font-black uppercase tracking-widest text-violet-400">Connect</button>
                           </div>
                         ))}
                      </div>
                   </div>
              ...
              {activeTab === "preferences" && (
                <motion.div
                  ...
                  className="space-y-8"
                >
                   <h1 className="text-3xl md:text-4xl font-black tracking-tighter uppercase italic text-white/90">Signal <span className="text-violet-500">Filters</span></h1>

                   <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-10 backdrop-blur-xl">
                      <div className="flex items-center justify-between">
                         <div className="space-y-2">
                             <h3 className="text-lg font-black uppercase tracking-tight">Stellar Updates</h3>
                             <p className="text-sm text-gray-500 font-medium max-w-sm">Receive transmissions about new course releases, community challenges, and mastery tips.</p>
                         </div>
                         <button 
                           onClick={() => handleUpdate({ newsletterSubscribed: !settings?.newsletterSubscribed })}
                         ...
                         >
                            <motion.div 
                              animate={{ x: settings?.newsletterSubscribed ? 32 : 0 }}
                              className="w-6 h-6 rounded-full bg-white shadow-lg"
                            />
                         </button>
                      </div>
              ...
              {activeTab === "billing" && (
                <motion.div
                  ...
                  className="space-y-8"
                >
                   <h1 className="text-3xl md:text-4xl font-black tracking-tighter uppercase italic text-white/90">Acquisition <span className="text-violet-500">Logs</span></h1>
                   ...
                   <div className="bg-slate-900/50 border border-white/5 border-dashed rounded-[3rem] p-20 text-center space-y-6">
                      ...
                      <h3 className="text-xl font-black uppercase tracking-tight text-white/60">No Active Subscriptions</h3>
                      <p className="text-sm text-gray-600 font-medium max-w-xs mx-auto">You haven't initiated any interstellar mastery plans yet.</p>
                      <button 
